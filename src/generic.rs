...
//! Data structures for lhe files, the init section and events
//!
//! This module contains data structures that hold all the mandatory
//! information specified by the lhe standard, with room to also hold
//! custom additional information for files generated by specific
//! programs.
...
/// A lhe file with generic additional information
///
...
pub struct LheFileGeneric<Comment, Header, InitExtra, EventExtra> {
    /// The version contained in the opening tag of the lhe file.
    pub version: String,
    /// The comment in the lhe file, in a custom format.
    pub comment: Comment,
    /// The header section in the lhe file, in a custom format.
    pub header: Header,
    /// The init section of the lhe file, with custom addition information.
    pub init: InitGeneric<InitExtra>,
    /// The events of the lhe file, each with custom addition information.
...
/// The init section of an lhe file
///
/// This struct can contain custom additional information that is
/// stored in the file.
///
...
pub struct InitGeneric<InitExtra> {
    /// The pdg id of beam 1 (`IDBMUP(1)`)
    pub beam_1_id: PdgId,
    /// The pdg id of beam 2 (`IDBMUP(2)`)
    pub beam_2_id: PdgId,
    /// The energy of beam 1 (`EBMUP(1)`)
    pub beam_1_energy: f64,
    /// The energy of beam 2 (`EBMUP(2)`)
    pub beam_2_energy: f64,
    /// PDF group id of beam 1 when using PDFLIB, 0 when using LHAPDF (`PDFGUP(1)`)
    pub beam_1_pdf_group_id: i64,
    /// PDF group id of beam 2 when using PDFLIB, 0 when using LHAPDF (`PDFGUP(2)`)
    pub beam_2_pdf_group_id: i64,
    /// PDF id of beam 1 (`PDFSUP(1)`)
    pub beam_1_pdf_id: i64,
    /// PDF id of beam 2 (`PDFSUP(2)`)
    pub beam_2_pdf_id: i64,
    /// Switch to indicate how event weights should be interpreted (`IDWTUP`)
    pub weighting_strategy: i64,
    /// `NPRUP` `ProcInfo` objects
    pub process_info: Vec<ProcInfo>,
    /// Any additional initialization information, in a custom format
    pub extra: InitExtra,
}
...
pub struct EventGeneric<EventExtra> {
    /// The id of the process this event belongs to (`IDPRUP`)
    pub process_id: i64,
    /// The weight of the event (`XWGTUP`)
    pub weight: f64,
    /// The scale of the event (`SCALUP`)
    pub scale: f64,
    /// Alpha electroweak (`AQEDUP`)
    pub alpha_ew: f64,
    /// Alpha strong (`AQCDUP`)
    pub alpha_qcd: f64,
    /// `NUP` particles
    pub particles: Vec<Particle>,
    /// Additional event information, in a custom format
